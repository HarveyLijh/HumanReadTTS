...
///
///     touch /tmp/rhea-run-network-tests && Scripts/test.sh
///
/// Or equivalently: `Scripts/test.sh --network`.
final class ModelCatalogReachabilityTests: XCTestCase {

    override func setUp() async throws {
        let sentinel = "/tmp/rhea-run-network-tests"
        try XCTSkipUnless(
            FileManager.default.fileExists(atPath: sentinel),
            "Network tests are opt-in. `touch /tmp/rhea-run-network-tests` and retry."
        )
    }
