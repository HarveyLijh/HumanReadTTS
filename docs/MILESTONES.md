...
- [ ] **M1.4** Sentence segmenter (Swift, using `NLTokenizer(unit: .sentence)` — the current API that supersedes `NSLinguisticTagger` for sentence tokenization; handles EN+ZH out of the box). Each sentence carries its owning block index and UTF-16 offset/length, which is enough for the M1.6 highlight layer to compute `PDFSelection`s on demand via `PDFDocument.findString` or page-level character bounds. Pre-computing PDF annotation coordinates for every sentence is deferred to M1.6.
- [ ] **M1.5** AVSpeechSynthesizer playback. Per-sentence queue with automatic voice selection via `NLLanguageRecognizer` (so EN/ZH paragraphs already pick a sensible voice before the Qwen3-TTS bilingual orchestrator lands in Month 3). A single circular play/pause button overlay on the document. Keyboard shortcut (Space) and richer transport (seek-to-sentence, ±15s) move to M2.6 when the menu work lands.
- [ ] **M1.6** Sentence highlight overlay on the PDF view. Amber `PDFAnnotationHighlight` placed per line of the active sentence's `PDFSelection`, removed when the next sentence starts; `PDFView.go(to:)` keeps it on screen. Sentence-level for M1.x; **word-level highlight (via WhisperKit forced-alignment timestamps from generated audio) lands with M2.3 / M2.4** since AVSpeechSynthesizer's `didSpeakRangeOfSpeechString` granularity is too coarse for our cadence.
- [ ] **M1.7** Library sidebar (basic). NavigationSplitView with a persistent recent-files list backed by `UserDefaults`-stored security-scoped bookmarks (so the same code works in sandboxed Release). Click to reopen. **Last-read-position persistence is deferred to M2** alongside the SwiftData migration per §2.
