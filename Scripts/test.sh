#!/usr/bin/env bash
#
# Run the Rhea test suite via xcodebuild and summarise results.
...
sentinel="/tmp/rhea-run-network-tests"
...
grep -E "Test Suite '[^']+' (passed|failed)" "$tmplog" \
    | grep -v "Selected tests\|All tests\|RheaTests.xctest" \
