#!/usr/bin/env bash
#
# Build a Release HumanReadTTS.app, sign it, and pack it into a .dmg.
#
...
usage() {
    cat <<EOF
Build a signed HumanReadTTS.dmg.
...
xcodebuild \
    -project "$PROJECT_ROOT/HumanReadTTS.xcodeproj" \
    -scheme "$SCHEME" \
...
#     1. Download and mount the DMG, drag HumanReadTTS to /Applications.
#     2. First launch: right-click HumanReadTTS.app → Open → click "Open" in
...
#     xattr -dr com.apple.quarantine /Applications/HumanReadTTS.app${c_reset}
EOF
fi
